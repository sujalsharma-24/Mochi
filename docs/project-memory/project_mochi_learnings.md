...
### After Session 9 (Login & Onboarding redesign)
- **Always cross-check new screen gradients against the existing design system (`Theme.kt`) before building.** The first Auth header used a deep violet-blue gradient that looked good in isolation but didn't match the rest of the app. Sujal caught it immediately on his physical device. The fix was simple (change 4 color hex values), but required a full rebuild + reinstall cycle. Would have been zero-cost to avoid: `MochiGradient.softButton` in `Theme.kt` is the canonical reference — match it before submitting any new gradient to device review.
- **Real brand logos (Google, Apple) must be PNG assets, not Canvas-drawn approximations.** The Canvas-drawn Google/Apple "logos" from the first attempt were visually wrong (wrong shapes, wrong colors). Using `generate_image` to create real-looking brand assets and saving them as PNGs in `drawable-nodpi/` is the right approach. Always use real assets for recognizable third-party brand logos rather than trying to draw them in code.
- **The `adb` binary is not in PATH on this machine — always use the full path:** `C:\Users\ACER\AppData\Local\Android\Sdk\platform-tools\adb.exe`. Every ADB command must use this full path. This will not change between sessions — add it as a fixed mental model, not something to re-discover.
- **The app's `applicationId` (`com.Adam.Mochi`) differs from the Kotlin `namespace` (`com.mochi.keyboard`).** Always launch via `com.Adam.Mochi/com.mochi.keyboard.MainActivity`, not `com.mochi.keyboard/com.mochi.keyboard.MainActivity`. This is a permanent project-specific quirk, not a bug to fix.
- **`SolidColor` must be imported explicitly in Compose files that use it for cursor brush.** It's easy to miss because it's not in the standard Compose `foundation` or `ui` imports — it lives in `androidx.compose.ui.graphics.SolidColor`. The Kotlin compiler catches it, but it cost one extra build cycle.
- **The build → ADB install → launch loop works reliably as the verification method for Android UI changes.** Confirmed again this session: Sujal can keep his phone connected and changes can be deployed end-to-end from this Windows machine without him needing to do anything manually, as long as USB Debugging is enabled and `adb devices` confirms the device.
- **Onboarding screens benefit far more from real PNG illustrations than icon drawables.** The redesigned screens with AI-generated `drawable-nodpi/` PNG artwork looked dramatically more premium than the previous icon-only version. For any new onboarding-style screen, generate real illustrations rather than using Material icons or small icon PNGs.
