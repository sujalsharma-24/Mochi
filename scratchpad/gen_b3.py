"""Generate BuiltInThemes+Batch3.swift — 40 new built-in themes from the freshly-ingested
themebg_*/keyart_* assets. Reuses the batch-2-refresh material system (7 materials, colours
derived from each plate)."""
...
OUT = "/Users/Tanmay/Desktop/Projects/Mochi/ios/MochiShared/Themes/BuiltInThemes+Batch3.swift"

# name -> material.  (plate stem is the kebab of the name; prefix from the folder)
THEMES = [
    # (Display Name, prefix, material)
    ("Aurora Frontier", "auf", "Inkwell"),
    ("Aurora Timber Lodge", "atl", "Clay"),
    ("Azure Summer Escape", "aze", "Pearl"),
    ("Beyond the Horizon", "bth", "Inkwell"),
    ("Bionova Nexus", "bnn", "Keycap"),
    ("Botanical Cafe", "bca", "Letterpress"),
    ("Canvas & Coffee", "cnc", "Clay"),
    ("Captured Moments", "cpm", "Clay"),
    ("Cozy Garden Cottage", "cgc", "Clay"),
    ("Creative Workspace", "crw", "Letterpress"),
    ("Deep Sea Explorer", "dse", "Pane"),
    ("Dreams in Rewind", "dir", "Keycap"),
    ("Emberwatch Observatory", "emo", "Inkwell"),
    ("Golden Midway", "gdm", "Jelly"),
    ("Iron & Ember Loft", "iel", "Keycap"),
    ("Japanese Zen", "jpz", "Letterpress"),
    ("Midnight Carnival", "mnc", "Inkwell"),
    ("Midnight Voyage", "mdv", "Pane"),
    ("Neon Roller Nights", "nrn", "Keycap"),
    ("Parisian Morning", "prm", "Pearl"),
    ("The Wanderers Cabin", "twc", "Inkwell"),
    ("Toymakers Memories", "tym", "Jelly"),
    ("Velvet Reel Reverie", "vrr", "Keycap"),
    ("Whispers of the Sewing Hearth", "wsh", "Clay"),
    ("Enchanted Midnight Library", "eml", "Inkwell"),
    ("Moonlit Meow Cafe", "mwc", "Pane"),
    ("Mystic Ocean Atelier", "moa", "Pane"),
    ("Neon Dream District", "ndd", "Jelly"),
    ("Sweetdream Carnival", "swc", "Jelly"),
    ("Wanderlust Scrapbook", "wls", "Clay"),
    ("Whispering Mushroom Cottage", "wms", "Clay"),
    ("Astral Gearworks", "asg", "Keycap"),
    ("Egypt Through Time", "egt", "Keycap"),
    ("Galaxy Mart", "gxm", "Jelly"),
    ("Midnight Racing Garage", "mrg", "Keycap"),
    ("Moonlit Hydrangea", "mhy", "Pearl"),
    ("Pirate Treasure Night", "ptn", "Inkwell"),
    ("Starlit 90s Haven", "s90", "Jelly"),
    ("Starlit Disco Reverie", "sdr", "Keycap"),
    ("Sunset Mirage", "sum", "Pearl"),
]
...
def sample(stem, anchor=0.5):
    ...
    # 3 vertical stops for baseFill
    stops=[tuple(band[int(winH*f), W//2]) for f in (0.15,0.5,0.85)]
    # busy metric: stdev of luminance in the core
...
def build_theme(name, pfx, material):
    ...
    # ink solved against worst surface
    def pp(top,bot):
    ...
    # chrome — the suggestion-bar / emoji-panel ink sits on the surface (scrim over baseFill), not
    # on a cap, so it needs its own solve. `.default(for: appearance)` assumes a near-black or
    # near-white backdrop; ours is often a mid tone, so author it explicitly.
    ...
    # WCAG crossover: black ink beats white above backdrop L≈0.18, below it white wins.
    ink_dark = bd_L >= 0.18
    chrome_ink = solve_ink(backdrop, want=6.5, prefer_dark=ink_dark)
    # verify; if the solve couldn't get there, take the pure extreme in the better direction
...
open("/Users/Tanmay/Desktop/Projects/Mochi/scratchpad/report_b3.txt","w").write("\n".join(rep))
print("\n".join(rep))
