"""Ingest THH (batch5, 19 themes) and Theses10 (batch6, 25 themes) into
ios/SharedAssets/KeyboardArt.xcassets, matching the batch2/3/4 asset conventions exactly."""
import os, json, subprocess, sys
from PIL import Image

SRC_DIRS = [
    "/Users/Tanmay/Downloads/THH",
    "/Users/Tanmay/Downloads/Theses10",
]
...
def main():
    ...
    report = []
    for d in SRC_DIRS:
        batch = "batch5" if "THH" in d else "batch6"
        for theme in sorted(os.listdir(d)):
            ...
            for f in files:
                ...
                ok = ingest_key(os.path.join(tdir, f), pfx, key)
                if ok: n += 1
                else: dropped += 1
            total += 1
            report.append((batch, theme, name, pfx, n, dropped))
            print(f"[{total:2d}] {batch} {theme:32s} pfx={pfx} keys={n} dropped={dropped}")
    print("done:", total, "themes")
    bad = [r for r in report if r[4] != 33]
    if bad:
        print("!! themes without exactly 33 keyart:", bad)
